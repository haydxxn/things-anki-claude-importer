...
function getConfig(): Config {
  const llmProvider = (process.env.LLM_PROVIDER ??
    "anthropic") as Config["llmProvider"];
  const llmModelDefault =
  ...
  return {
    ...
    ankiTags: (process.env.ANKI_TAGS ?? "things,vocab")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean),
    llmProvider,
...
function buildThingsListTodosScript(params: {
  tagToWatch: string;
  tagProcessed: string;
}): string {
...
async function thingsListTodos(params: {
  tagToWatch: string;
  tagProcessed: string;
}): Promise<ThingsTodo[]> {
  ...
  const parsed = z
    .array(
      z.object({ id: z.string().min(1), title: z.string(), notes: z.string() }),
    )
    .parse(JSON.parse(stdout.trim() || "[]"));
  return parsed;
}

function buildThingsMarkCompletedScript(params: {
  todoId: string;
  watchTagName: string;
}): string {
  const { todoId, watchTagName } = params;
  return `
...
async function thingsMarkCompleted(params: {
  todoId: string;
  watchTagName: string;
}): Promise<void> {
...
async function ankiHasBasicFront(params: {
  deckName: string;
  front: string;
}): Promise<boolean> {
...
function formatBack(params: {
  ...
  const partOfSpeech = params.partOfSpeech
    ? escapeHtml(params.partOfSpeech)
    : null;
  ...
  const synonyms = params.synonyms?.length
    ? params.synonyms.map((s) => escapeHtml(s))
    : null;

  const rows: string[] = [];
  rows.push(`<div><b>IPA</b>: /${ipa}/</div>`);
  if (partOfSpeech)
    rows.push(`<div><b>Part of speech</b>: ${partOfSpeech}</div>`);
  rows.push(
    `<div style="margin-top:10px"><b>Definition</b><br>${definition}</div>`,
  );
  rows.push(`<div style="margin-top:10px"><b>Example</b><br>${example}</div>`);

  if (synonyms) {
    rows.push(
      `<div style="margin-top:10px"><b>Synonyms</b><br>${synonyms.join("<br>")}</div>`,
    );
  }

  if (notes)
    rows.push(`<div style="margin-top:10px"><b>Notes</b><br>${notes}</div>`);
...
async function generateCardViaOpenRouter(params: {
  term: string;
  context?: string;
  model: string;
  maxTokens: number;
  ...
  const prompt = [
    ...
    params.context?.trim()
      ? `context: ${JSON.stringify(params.context.trim())}`
      : "",
  ]
    .filter(Boolean)
    .join("\n");
...
async function main() {
  ...
  const todos = await thingsListTodos({
    tagToWatch: config.thingsTagToWatch,
    tagProcessed: config.thingsTagProcessed,
  });
  ...
  for (const todo of todos) {
    ...
    const alreadyInAnki = await ankiHasBasicFront({
    ...
    const card =
      config.llmProvider === "openrouter"
        ? await generateCardViaOpenRouter({
            term,
            context: todo.notes,
