...
async function main() {
  ...
  const templatePath = resolve(
    repoDir,
    "launchagent",
    "com.things-anki-claude.plist.template",
  );
  const template = await readFile(templatePath, "utf8");

  const rendered = template.replaceAll("__REPO_DIR__", repoDir);
  if (rendered.includes("__REPO_DIR__"))
    throw new Error("Failed to render LaunchAgent template.");
  ...
  process.stdout.write(
    "Schedule: runs every 2 hours (see StartInterval in the plist).\n",
  );
  process.stdout.write(
    "Next: edit it to add your API key(s), then `launchctl load` it.\n",
  );
}
